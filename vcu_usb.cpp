...
// CAN Frame Structure
struct can_frame {
    uint32_t can_id; // 32-bit CAN ID (standard or extended)
...
public:
    ...
    IntegratedController(const std::string& device = "/dev/ttyUSB1")
        : device_(device), running_(true), nmt_state_(NMT_State::INITIALIZING),
          last_sync_time_(0), rpdo1_timer_(0), currentAddress_(0), fd_(-1) {
        initialize_serial();
        start_threads();
    ...
    bool sendPduCommand(const can_frame& frame, PduOpCode responseOpCode, 
                        can_frame* response, int timeoutMs = 1000) {
        CanMessage msg{
            .priority = 3,
            .frame = frame
        };
        ...
        std::cout << "Sending PDU command: ID=0x" << std::hex << frame.can_id 
                  << ", OpCode=0x" << static_cast<int>(frame.data[0]) << std::dec << std::endl;
        ...
        if (responseOpCode != static_cast<PduOpCode>(0) && response != nullptr) {
            ...
            if (received) {
                *response = responses_[static_cast<PduOpCode>(responseOpCode)];
                ...
                std::cout << "Received PDU response for OpCode=0x" << std::hex 
                          << static_cast<int>(responseOpCode) << std::dec << std::endl;
                return true;
            }
            std::cerr << "Timeout waiting for PDU response: OpCode=0x" << std::hex 
                      << static_cast<int>(responseOpCode) << std::dec << std::endl;
            return false;
        }
...
private:
    ...
    std::map<PduOpCode, can_frame> responses_;
    std::map<PduOpCode, std::vector<std::function<void(const can_frame&)>>> alert_callbacks_;
    ...
    bool initialize_serial() {
        ...
        std::cout << "Serial port opened on " << device_ << " at 1000000 bps" << std::endl;
        return true;
    }

    bool send_serial_frame(const can_frame& frame) {
        if (fd_ == -1) {
        ...
        std::lock_guard<std::mutex> lock(send_mutex_);
        ssize_t written = write(fd_, serial_frame.data(), serial_frame.size());
    ...
    bool receive_serial_frame(can_frame& frame) {
        uint8_t byte;
        ssize_t n;
        ...
        while (true) {
            ...
            if (receive_buffer_.size() >= 5 && receive_buffer_[0] == 0xAA && receive_buffer_.back() == 0x55) {
                size_t pos = 0;
                if (receive_buffer_[pos] != 0xAA) {
                    receive_buffer_.clear();
                    continue;
                }
                pos++;

                uint8_t frame_info = receive_buffer_[pos++];
                uint8_t dlc = frame_info & 0x0F;
                bool extended = (frame_info & 0x20) || frame_info == 0xc2;

                if (dlc > 8 || (extended && receive_buffer_.size() < 7 + dlc) || (!extended && receive_buffer_.size() < 5 + dlc)) {
                    receive_buffer_.clear();
                    continue;
                }
                ...
                frame.can_dlc = dlc;
                if (extended) {
                    frame.can_id = (receive_buffer_[pos] |
                                    (receive_buffer_[pos+1] << 8) |
                                    (receive_buffer_[pos+2] << 16) |
                                    (receive_buffer_[pos+3] << 24));
                    pos += 4;
                } else {
                    frame.can_id = (receive_buffer_[pos] |
                                    (receive_buffer_[pos+1] << 8));
                ...
                for (uint8_t i = 0; i < dlc; i++) {
                    frame.data[i] = receive_buffer_[pos++];
                }

                if (receive_buffer_[pos] != 0x55) {
                    receive_buffer_.clear();
                    continue;
                }

                // Log received frame
                std::cerr << "Received CAN frame: ID=0x" << std::hex << std::setw(8) << std::setfill('0') << frame.can_id
                          << ", DLC=" << std::dec << (int)dlc << ", Data=[";
                for (uint8_t i = 0; i < dlc; i++) {
                    std::cerr << std::hex << std::setw(2) << std::setfill('0') << (int)frame.data[i] << (i < dlc-1 ? " " : "");
                }
    ...
    void start_threads() {
        send_thread_ = std::thread([this]() {
            while (running_) {
                ...
                if (queue_size > 10) {
                    std::cerr << "Warning: Send queue has " << queue_size << " messages, flushing old messages" << std::endl;
                    while (!send_queue_.empty()) send_queue_.pop();
                }
                while (!send_queue_.empty()) {
                    auto msg = send_queue_.top();
                    send_queue_.pop();
                    std::cerr << "Dequeuing CAN frame: ID=0x" << std::hex << std::setw(8) << std::setfill('0') 
                              << msg.frame.can_id << ", Priority=" << (int)msg.priority << std::dec << std::endl;
                    lock.unlock();
                    if (!send_serial_frame(msg.frame)) {
                        std::cerr << "Failed to send CAN frame ID=0x" << std::hex << msg.frame.can_id << std::dec << std::endl;
                    }
                    lock.lock();
                }
            }
        ...
        receive_thread_ = std::thread([this]() {
            while (running_) {
                can_frame frame;
                if (receive_serial_frame(frame)) {
                    if (frame.can_id == PDU_RESPONSE_ID || frame.can_id == PDU_ALERT_ID) {
    ...
    void register_default_alert_callbacks() {
        register_alert_callback(static_cast<PduOpCode>(TRIP_ALERT), [this](const can_frame& frame) {
    ...
    void register_alert_callback(PduOpCode alertOpCode, std::function<void(const can_frame&)> callback) {
        alert_callbacks_[alertOpCode].push_back(callback);
    }
    ...
    void process_pdu_frame(const can_frame& frame) {
        if (frame.can_id == PDU_RESPONSE_ID) {
            ...
            std::cout << std::dec << std::endl;
            uint8_t opCode = frame.data[0];
            if (alert_callbacks_.find(static_cast<PduOpCode>(opCode)) != alert_callbacks_.end()) {
                for (auto& callback : alert_callbacks_[static_cast<PduOpCode>(opCode)]) {
                    callback(frame);
                }
            }
        }
    ...
    void process_vcu_frame(const can_frame& frame) {
        ...
        std::cout << "Processing VCU frame: ID=0x" << std::hex << std::setw(3) << std::setfill('0') << frame.can_id 
                  << ", DLC=" << std::dec << (int)frame.can_dlc << ", Priority=" << (int)priority << std::endl;
        switch (frame.can_id) {
            ...
            default:
                std::cout << "Unhandled VCU frame: ID=0x" << std::hex << frame.can_id << std::dec << std::endl;
        }
    }
    ...
    void handle_tpdo1(const can_frame& frame) {
        if (frame.can_dlc < 5) {
            std::cerr << "TPDO1 invalid DLC: " << (int)frame.can_dlc << std::endl;
            return;
        }
        ...
        std::cout << "Received TPDO1 (ID=0x" << std::hex << TPD01_ID << std::dec << "): "
                  << "RPM=" << rpm << ", Current=" << (int)current << "A, Temp=" << (int)temp 
                  << "°C, Speed=" << speed << std::endl;
    }

    void handle_tpdo2(const can_frame& frame) {
        if (frame.can_dlc < 4) {
            std::cerr << "TPDO2 invalid DLC: " << (int)frame.can_dlc << std::endl;
            return;
        }
        ...
        std::cout << "Received TPDO2 (ID=0x" << std::hex << TPD02_ID << std::dec << "): "
                  << "Voltage=" << voltage << "V, Current=" << current << "A, SOC=" << (int)soc 
                  << "%, Controller Temp=" << (int)temp << "°C" << std::endl;
    }
...
int main() {
    try {
        IntegratedController controller("/dev/ttyUSB1");
            ...
            // VCU Commands
                ...
                try {
                    if (tokens[1].length() == 4) {
                        ...
                        std::cout << "Sent NMT command: 0x" << std::hex << (int)nmt_cmd 
                                  << " to node " << (int)node_id << std::dec << "\n";
                    } else {
